...
export default function Page() {
  ...
  return (
    <main
      className={`main-box fixed  ${darkMode ? 'bg-gray-800 text-gray-100' : 'bg-gray-50 text-gray-800'
        } flex items-center justify-center w-full h-screen font-sans`}
    >

      <div className={`fixed top-2 right-2 z-[5000000] rounded-full ${darkMode ? "bg-gray-00" : "bg-gray-00"}`}>
        <Button
          onClick={() => setDarkMode(!darkMode)}
          className={`flex items-center gap-2 text-[12px] rounded-full border ${darkMode ? 'text-gray-400 border-gray-700' : ' text-gray-400 border-gray-300'
            }`}
        >
          {darkMode ? (
            <span className="flex items-center justify-center w-2 h-2">☀️</span>
          ) : (
            <span className="flex items-center justify-center w-2 h-2">🌒</span>
          )}
        </Button>
      </div>
      ...
      <div className={`relative h-[96vh] w-full flex items-center justify-center`}>
        <div
          className={`relative overflow-y-auto h-[96vh] w-full max-w-4xl border ${darkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-00'
            } rounded-lg`}
        >
          {/* Interactive Banner */}
          ...
          <div className="max-w-2xl mx-auto px-4 -mt-16 relative z-10">
            <div className="relative flex flex-col items-center text-center mb-12">
              <div className="relative rounded-full border-4 border-white shadow-lg overflow-hidden w-32 h-32 mb-4">
                <Image
            ...
            <Tabs defaultValue="feed" className="mb-12">
              <TabsList className={`grid w-full font-[400] grid-cols-4 bg-gray-0 border ${darkMode ? "border-gray-700" : ""} rounded-[10px] text-gray-400`}>
                <TabsTrigger
                  value="feed"
              ...
              <TabsContent value="about" className="mt-6 animate-fadeIn">
                ...
                <div className={`prose dark:prose-invert max-w-none border ${darkMode ? "border-gray-700" : ""} rounded-lg p-6`}>
              ...
              <TabsContent value="content" className="animate-fadeIn">
                <div className="grid gap-6 md:grid-cols-2">
                  {[
                    ...
                    <div
                      key={index}
                      className={`relative h-[280px] w-[300px] bg-transparent p-3 rounded-[20px] border ${darkMode ? "border-gray-700" : ""} transition-shadow duration-300 group overflow-hidden`}
                    >
                      {/* Image */}
                      <Image
                        src={item.image}
                      ...
                      <div className={`relative z-10 h-full ${darkMode ? "text-gray-800" : ""} flex flex-col justify-end`}>
                        <Link href={item.link}>
                ...
                <div className="space-y-6">
                  ...
                  <div className="space-y-4">
                    {guestbookEntries.map((entry, index) => (
                      <div key={index} className={`bg-gray-00 ${darkMode ? "bg-transparent border-gray-700" : ""} p-4 rounded-lg shadow-sm border`}>
